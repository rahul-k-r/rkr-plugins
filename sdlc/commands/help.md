...
| **story-run / design-run** | `--turbo` | Fewer dispatches, tighter budgets, same models: skips the design lens panel, the architect's plan review, and the per-batch `reviewer` (the validator still gates every batch; the story-level final review still runs). Budgets become retries 2 / replans 0 / revision cycles 1 — explicit `--max-retries`/`--max-replans` still win. Persisted in state; can't be toggled on `--resume`. For small, well-understood stories — a cross-batch defect now surfaces only at the final review. |
