...
|---------|-----------|-------|
| **/sdlc:init** | Bootstrap a repo for this workflow: `docs/design-notes/`, `docs/adr/`, template, process pointer, and `.sdlc/config.json` (tracker + branch model + model effort). Idempotent. | `/sdlc:init` |
...
- **Model-effort tiering: also configured per repo, optional.** `.sdlc/config.json`'s `modelEffort` (`very-low|low|medium|high|extra-high`, default `high`) sets which model tier every subagent dispatch runs at; `/sdlc:story-run`'s `--effort <tier>` overrides it for a single run. `/sdlc:init` asks and persists this alongside tracker/branch model. Full resolution order and the per-agent tier table: `skills/model-effort/SKILL.md`.
...
| **story-run / design-run / review-run / review-fix / product-design-review** | `--effort <tier>` | Override `.sdlc/config.json`'s `modelEffort` for this run only (`very-low\|low\|medium\|high\|extra-high`), per `skills/model-effort/SKILL.md`. Omitted → the config's `modelEffort`, or `high` if that's unset too. A run dispatched inline by another (e.g. `design-run` from `story-run`, or the `--bypass` tail's inline `review-fix`) inherits the parent's already-resolved tier rather than re-resolving; read back (not re-resolved) on `--resume`. |
