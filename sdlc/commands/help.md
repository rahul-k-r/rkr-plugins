...
**Engineer-level voice by default:** the commands that explain, deliberate, or gate — `story-run`, `design-run`, `review-run`, `review-fix`, `plan-the-design`, `design-review`, `story-start`, `story-pr`, `story-check`, `sprint-pr`, `close-story` — accept `--plain`. With it, their chat-facing explanations follow the plain-language standard (`internal/plain-language/STANDARD.md`) so less technical users can drive the workflow; without it they keep the engineer-level voice. Artifacts (commits, PRs, tracker comments, design notes) are identical in both modes. The mechanical commands (`adr`, `build-check`, `commit`, `help`, `init`, `show-stats`) don't take the flag — standalone they keep their normal voice, and inside a run they inherit the run's mode.
...
- **Effort tiering: also configured per repo, optional.** `.sdlc/config.json`'s `effort` (`very-low|low|medium|high|extra-high`, default `high`) sets which model tier every subagent dispatch runs at; `/sdlc:story-run`'s `--effort <tier>` overrides it for a single run. `/sdlc:init` asks and persists this alongside tracker/branch model. Full resolution order and the per-agent tier table: `internal/model-effort/SKILL.md`.
- **Worktree isolation: resolved per run, config optional.** `/sdlc:story-run` and `/sdlc:story-start` isolate a story's git/file work in its own `git worktree` by default; `--worktree <path>` targets an existing one instead of creating it, `--no-worktree` works directly in the current checkout. A freshly created worktree is bootstrapped once at creation — dependency installs auto-detected from its lockfiles (npm/yarn/pnpm/bun, uv/poetry/pipenv, bundler, composer) plus submodules; `.sdlc/config.json`'s optional `worktreeSetup` list overrides that for stacks auto-detection can't map, and a run offers to persist it the first time it has to ask. Full resolution, bootstrap, fallback, and recovery logic: `internal/worktree-mode/SKILL.md`.
- **Local docs: no config default needed, opt-in per repo.** `.sdlc/config.json`'s `localDocs` (default `false`) — `true` means design notes/ADRs are written but never staged or committed, staying local-only forever. Full resolution and behavior: `internal/local-docs/SKILL.md`.
...
- **`validate-state.js`** (PostToolUse on Edit/Write) — validates any written `story-state.json` (parseable, legal phase, key fields, and every `budgets.*_used` counter within its `max_*`) so audit-trail corruption is caught the moment it happens.
...
|---------|-------------|--------------|
| **runs + plan/review/PR/lifecycle commands** | `--plain` | Narrate chat output per the plain-language standard (`internal/plain-language/STANDARD.md`) instead of the default engineer-level voice; durable artifacts are identical either way. The runs persist the mode in their state file so `--resume` keeps it, and it cascades into every procedure a run executes inline (design-run phase, the whole `--bypass` tail, `--close`'s sprint-pr). |
...
| **story-run / story-start / design-run** | `--worktree <path>` / `--no-worktree` | How the story's git/file work is isolated, per `internal/worktree-mode/SKILL.md`. Default (neither passed): create a fresh `git worktree` at `.claude/worktrees/<key-lower>-slug`. `--worktree <path>` targets an existing one instead of creating it; `--no-worktree` skips isolation and operates directly in the current checkout. Mutually exclusive. A `design-run` dispatched inline by `story-run` inherits the parent's resolution instead of re-resolving; `review-fix` looks up an existing story's `worktree` from its state file but never creates or takes these flags itself, since it operates on an already-open PR. |
| **story-run / design-run / review-run / review-fix / product-design-review** | `--effort <tier>` | Override `.sdlc/config.json`'s `effort` for this run only (`very-low\|low\|medium\|high\|extra-high`), per `internal/model-effort/SKILL.md`. Omitted → the config's `effort`, or `high` if that's unset too. A run dispatched inline by another (e.g. `design-run` from `story-run`, or the `--bypass` tail's inline `review-fix`) inherits the parent's already-resolved tier rather than re-resolving; read back (not re-resolved) on `--resume`. |
| **story-run** | `--max-retries N` / `--max-replans N` | Budgets before automatic escalation (default 4 / 2 — up to 5 attempts per batch). |
| **story-run** | `--turbo` | Fewer dispatches, tighter budgets, same models: skips the design lens panel, the architect's plan review, and the per-batch `reviewer` (the validator still gates every batch; the story-level final review still runs). Budgets become retries 2 / replans 0 / revision cycles 1 — see `story-run.md`'s `--turbo` for the exact numbers; explicit `--max-retries`/`--max-replans` still win. Persisted in state; can't be toggled on `--resume`. For small, well-understood stories — a cross-batch defect now surfaces only at the final review. |
| **design-run** (also standalone) | `--turbo` | Skips the lens panel (every issue goes designer-with-evidence) and sets both revision budgets to 1 — see `design-run.md`'s `--turbo` for the exact numbers. No batches/retries/replans in design-run, so those don't apply here. |
