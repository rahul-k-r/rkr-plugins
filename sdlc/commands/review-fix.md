...
- `--effort <tier>` — override `.sdlc/config.json`'s `modelEffort` for this run only, per `skills/model-effort/SKILL.md` (`very-low`/`low`/`medium`/`high`/`extra-high`). Omitted → the config's `modelEffort`, or `high` if that's unset too. **When invoked inline (`review-run --fix`, `story-run --bypass`'s `AUTO_FIX` step), `model_effort` was already resolved by the caller — inherited as given, never re-resolved.**
...
0. **Preflight.** `gh` authenticated; project config resolved from the target repo's `CLAUDE.md` (verify commands, mandatory rules, required CI check names). **Resolve `model_effort`** per `skills/model-effort/SKILL.md`: when invoked inline (`review-run --fix`, `story-run --bypass`'s `AUTO_FIX` step), it's already resolved by the caller — inherit it as given, never re-resolve. Standalone: `--effort <tier>` if passed, else `.sdlc/config.json`'s `modelEffort`, else `high`. Every subagent dispatch below (`assessor`, `coder`, `validator`, `pr-reviewer`) passes an explicit `model` looked up from that skill's table for this tier — not restated at each dispatch below. `gh pr view --json number,title,author,baseRefName,headRefName,state,mergedAt` — enforce the **Hard boundaries** above. Working tree must be clean (dirty → stop and say why; never stash someone's work). Record the currently checked-out branch to restore at the end when this run had to switch.
