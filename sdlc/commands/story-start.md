...
argument-hint: "<STORY-KEY> [--incognito] [--technical] (e.g. AGL-19)"
---
...
6. **Create or confirm the story branch.** Name it by the kind of work, slug from the story summary (or `local_ac`): `feat/<key-lower>-slug` (new functionality), `fix/<key-lower>-slug` (bug fix), or `chore/<key-lower>-slug` (tooling/hygiene) — or the repo's own convention from `CLAUDE.md`. If the branch already exists, confirm it is based on the current base head. If not, `git switch -c <branch>` from the base. Never force-push or reset existing branches.

6a. **Commit the design note onto the branch.** `/sdlc:plan-the-design` wrote `docs/design-notes/$1.md` before this branch existed, so it's likely still untracked (or, on an existing branch, may have uncommitted edits from a later `/sdlc:design-review` pass). Check `git status docs/design-notes/$1.md docs/adr`: if anything is untracked or modified, `git add` it and commit now (`git commit -m "$1: add design note"`) so it ships as part of this story's history from the start. Never leave it to ride along accidentally in a later commit.
