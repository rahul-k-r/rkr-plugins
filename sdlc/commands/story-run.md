...
The run is autonomous by default. It stops and asks the developer (in chat, with an audit copy posted to the tracker — or the local provenance file, under incognito — as an `ESCALATION` record per `skills/tracker-adapter/SKILL.md`) **only** for:
...
- `--effort <tier>` — override `.sdlc/config.json`'s `effort` for this run only, per `skills/model-effort/SKILL.md` (`very-low`/`low`/`medium`/`high`/`extra-high`). Omitted → the config's `effort`, or `high` if that's unset too.
...
0. **Preflight.**
   ...
   - **Resolve `effort`** per `skills/model-effort/SKILL.md`: `--effort <tier>` if passed, else `.sdlc/config.json`'s `effort`, else `high`. Every subagent dispatch for the rest of this run passes an explicit `model` argument looked up from that skill's table for this tier — not restated at each dispatch below.
   ...
   - **Resolve `local_docs`** per `skills/local-docs/SKILL.md`: `.sdlc/config.json`'s `localDocs`, default `false`. `true` changes where design notes/ADRs live (session's own root, never the worktree) and means Step 2 never stages or commits them — see that step.
...
5. **IMPLEMENT LOOP** (`phase: IMPLEMENT`). For the next batch with `status != DONE`:
   ...
   d. **Verdict — decided here, by the orchestrator, not a dispatch.** Every input is already in hand (plan, batch spec, coder output, reviewer findings, validator results, budgets — nothing to re-send), and you never saw the coder's reasoning either, only its structured output, so the blind-review property holds. Apply the procedure in order:
      1. Any validator step at `FAIL_ENV` → **ESCALATE**, not retry: the worktree's environment is broken (dependencies not installed, wrong venv, a tool missing from `PATH`), not the code — the coder can't fix that by editing source, and each retry would burn budget while the cause stays hidden. Name the step, the excerpt, and the likely fix (re-run the bootstrap for that directory, or add a `worktreeSetup` entry — `skills/worktree-mode/SKILL.md`).
...
- See `skills/tracker-adapter/SKILL.md` for the full tracker/write-mode resolution logic referenced throughout, `skills/model-effort/SKILL.md` for the full model-tier table and resolution, `skills/worktree-mode/SKILL.md` for the full worktree resolution/fallback/recovery logic, and `skills/local-docs/SKILL.md` for the full design-note/ADR locality logic — this file only describes how story-run *uses* each, not how they work.
