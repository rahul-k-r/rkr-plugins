...
argument-hint: "[<KEY>] [--base <branch>] [--worktree <path> | --no-worktree] [--effort <tier>] [--max-retries N] [--max-replans N] [--turbo] [--gate <phase,...>] [--no-gate <phase,...>] [--bypass] [--review] [--incognito] [--plain] [--show-stats] [--resume]"
---

# /sdlc:story-run

Runs the full story lifecycle autonomously — design note (if none exists), story setup, implementation plan, batch-by-batch coding with review and validation, a story-level final review, and the PR to the base branch — orchestrated from this live session using a fixed set of subagents (`agents/designer.md`, `architect.md`, `intake.md`, `planner.md`, `coder.md`, `reviewer.md`, `validator.md`), each dispatched fresh via the Task tool so no reviewer ever inherits the reasoning of the agent it judges. Batch verdicts (PROCEED/RETRY/REPLAN/ESCALATE) and provenance posting are the orchestrator's own work, not dispatches — they consume only what it already holds. The session itself is the orchestrator (subagents cannot dispatch subagents, and only the session can put a question to the developer), so it operates with the judgment of a highly experienced engineer: it decides what it has evidence for, and it **asks the developer only where judgment is genuinely theirs** — everything else runs unattended. By default it ends by informing the developer that the PR is open and ready for their review; closing the story stays human (`/sdlc:close-story`). With `--bypass`, it instead keeps going — auto-review, merge, and close — see that flag below. With `--incognito` (or whenever no tracker is configured at all), it never pushes automatically — see **`--incognito` and no-ticket runs** below.
...
The run is autonomous by default. It stops and asks the developer (in chat, with an audit copy posted to the tracker — or the local provenance file, under incognito — as an `ESCALATION` record per `internal/tracker-adapter/SKILL.md`) **only** for:
...
```
/sdlc:story-run [<KEY>] [--base <branch>] [--worktree <path> | --no-worktree] [--effort <tier>] [--max-retries N] [--max-replans N] [--turbo] [--gate <phase,...>] [--no-gate <phase,...>] [--bypass] [--review] [--incognito] [--plain] [--show-stats] [--resume]
```
...
- `--worktree <path>` / `--no-worktree` — how the story's git/file work is isolated, per `internal/worktree-mode/SKILL.md`. Default (neither passed): create a fresh `git worktree` at `.claude/worktrees/<key-lower>-slug`. `--worktree <path>` targets an existing one instead of creating it. `--no-worktree` skips worktree isolation entirely, operating directly in the current checkout. Mutually exclusive with each other. This session never calls `EnterWorktree` itself for any of the three — see that skill for why.
- `--effort <tier>` — override `.sdlc/config.json`'s `effort` for this run only, per `internal/model-effort/SKILL.md` (`very-low`/`low`/`medium`/`high`/`extra-high`). Omitted → the config's `effort`, or `high` if that's unset too.
...
- `--turbo` — fewer dispatches and tighter budgets for a small, well-understood story; models stay at the repo's `effort` tier. Skips exactly three things: the DESIGN phase's lens panel (every issue goes the designer-with-evidence route — design-run's own `--turbo`), the `architect`'s PLAN_REVIEW (the plan is taken as planned; `--gate plan` still routes it to you), and the per-batch `reviewer` (Step 5b — `validator` still gates every batch; the FINAL REVIEW's story-level pass becomes the first and only review of the diff). Budgets: `max_retries_per_batch: 2`, `max_replans: 0` (any REPLAN is an ESCALATE), and the three revision budgets at `1` — an explicit `--max-retries`/`--max-replans` still wins over these. Nothing else changes: verdict rules, escalations, the design gate, the final review, the hooks. Recorded as `turbo: true` in state and read back on `--resume`. The trade: with no per-batch review, a cross-batch defect surfaces only at FINAL_REVIEW, where the fix is bigger than the review it skipped — wrong for anything touching a shared contract. Default: off.
...
- `--review` — **only meaningful with `--bypass`** (this run never invokes `review-run`/`review-fix` without it, so the flag is otherwise a no-op). Passed straight through to the bypass tail's AUTO_REVIEW step, which runs it as `review-run <PR#> --review` — a `/code-review` pass alongside `reviewer`'s, per `review-run.md`'s step 2a — and, if AUTO_FIX runs, to its inline `review-fix` procedure the same way, so its convergence check re-verifies via `/code-review` too. Recorded in state, read back on `--resume`. Default: off.
...
- **Every write goes to `docs/stories/<KEY>/provenance.md`** instead of the tracker: the plan digest, each batch record, escalations, review notes, the completion record — same structured entries that would otherwise be posted to the tracker (`internal/tracker-adapter/SKILL.md` → **Provenance records**), just local — status transitions and assignments (Step 1) included, recorded as "would have transitioned `<key>` to In Progress"-style entries rather than performed. This one rule covers every write in the steps below, which don't restate it. It lives under `docs/stories/<KEY>/`, already untracked by the `.gitignore` Step 3 ensures — confirm that's actually in place before pushing anything, since leaking your own working notes into a diff you send to a repo you don't own is exactly what this exists to prevent.
...
- **Discovered-work triage** (Step 8) never creates or comments on a real ticket — "new story" becomes a local backlog entry in the provenance file, and "comment on an existing ticket" isn't offered.

## Steps

0. **Preflight.**
   - **Resolve the tracker and write-mode** per `internal/tracker-adapter/SKILL.md`: read `.sdlc/config.json` (or detect the registered MCP family and ask, offering to persist the choice). Resolve `writeMode`: `--incognito` passed → `incognito`; else `tracker: none` → `incognito`; else `normal`. **If both `--incognito` and `--bypass` were passed, stop immediately and say why** — they're incompatible. `tracker: none` is never itself a reason to stop; it just means every write for this run goes local (see **`--incognito` and no-ticket runs**).
   - **Resolve `effort`** per `internal/model-effort/SKILL.md`: `--effort <tier>` if passed, else `.sdlc/config.json`'s `effort`, else `high`. Every subagent dispatch for the rest of this run passes an explicit `model` argument looked up from that skill's table for this tier — not restated at each dispatch below.
   - **Resolve worktree mode** per `internal/worktree-mode/SKILL.md`: `--worktree <path>` / `--no-worktree` / default (create one). This session never calls `EnterWorktree` for any of the three modes — worktree creation, when it happens, is a plain `git worktree add`, and every git/file operation that needs to reach it does so via path-qualified commands, never a session-level directory change. Resolved here but *applied* in Step 1, once the branch/key are known (the worktree's directory name needs the slug).
   - **Resolve `local_docs`** per `internal/local-docs/SKILL.md`: `.sdlc/config.json`'s `localDocs`, default `false`. `true` changes where design notes/ADRs live (session's own root, never the worktree) and means Step 2 never stages or commits them — see that step.
...
1. **Story setup.** If a real tracker key resolved in Step 0, fetch the issue (full AC) and confirm it's a real story in the tracker's project and not Done. Otherwise (no-ticket run), skip the fetch — resolve the local key and `local_ac` per **`--incognito` and no-ticket runs** above. Initialize `story-state.json` immediately (phase `DESIGN`, plus `tracker`, `write_mode`, `local_key`, `turbo`, and the `budgets` block — the schema's defaults, or `--turbo`'s, with any explicit `--max-retries`/`--max-replans` applied last) — every later pause needs somewhere to persist, including pauses before any other artifact exists.
...
4. **PLAN** (`phase: PLAN`). Dispatch `planner` with the context pack; it writes `docs/stories/<KEY>/plan-summary.md` and the `plan` block. Initialize `story-state.json` with the plan.
   ...
   **Plan sign-off:** **under `--turbo`, no PLAN_REVIEW** — record `gates.plan: {status: APPROVED, mode: turbo}` and proceed (`--gate plan` still routes it to the developer instead, below). Otherwise, by default, dispatch `architect` with directive `PLAN_REVIEW`. APPROVE → record `gates.plan: {status: APPROVED, mode: architect}` and proceed. REVISE → back to `planner`, incrementing `budgets.plan_revisions_used`; one that would exceed `max_plan_revisions` is an ESCALATE instead (budget exhaustion — see design-run's **Revision budgets**). ESCALATE → ask the developer. With `--gate plan`, show `plan-summary.md` to the developer instead and proceed only on explicit approval (`mode: human`). Either way, post the `PLAN_APPROVED` record yourself (`internal/tracker-adapter/SKILL.md` → **Provenance records**) — the plan digest (subtask/batch table, approval mode); that record is the plan's provenance — `plan-summary.md` itself is ephemeral working state.

5. **IMPLEMENT LOOP** (`phase: IMPLEMENT`). For the next batch with `status != DONE`:

   ```
   coder(batch) ──> reviewer(diff) ──> validator(gate) ──> verdict (orchestrator)
        ▲                                                        │
        │ RETRY (fix-list, same batch, attempt+1)                │ verdict
        │◄───────────────────────────────────────────────────────┤
        │ REPLAN ──> planner (remaining subtasks only)           │
        │ ESCALATE ──> ask developer; audit copy posted          │
        │ PROCEED ──> next batch (last one → FINAL REVIEW)       │
   ```

   a. If this is the batch's **first** attempt, record `git rev-parse HEAD` in `batch_base_shas[<batch>]` — fixed for the life of the batch so retries are reviewed cumulatively. Dispatch `coder` with the batch spec (and, on retry, the fix-list from 5d). It commits locally, one commit per subtask, staging only files it touched — never pushes, never stages `docs/stories/`.
   b. Dispatch `reviewer` with `git diff <batch_base_sha>..HEAD` and the context pack only — **not** the coder's summary or reasoning. (`docs/stories/` is untracked, so the diff is pure implementation.) **Skipped under `--turbo`** — record `review: null` in the batch result; the verdict in 5d is then decided on the validator and coder output alone (its reviewer-finding rules have nothing to fire on), and Step 6's story-level pass is the first review the diff gets.
   c. Dispatch `validator` with the resolved verify commands.
   d. **Verdict — decided here, by the orchestrator, not a dispatch.** Every input is already in hand (plan, batch spec, coder output, reviewer findings, validator results, budgets — nothing to re-send), and you never saw the coder's reasoning either, only its structured output, so the blind-review property holds. Apply the procedure in order:
      1. Any validator step at `FAIL_ENV` → **ESCALATE**, not retry: the worktree's environment is broken (dependencies not installed, wrong venv, a tool missing from `PATH`), not the code — the coder can't fix that by editing source, and each retry would burn budget while the cause stays hidden. Name the step, the excerpt, and the likely fix (re-run the bootstrap for that directory, or add a `worktreeSetup` entry — `internal/worktree-mode/SKILL.md`).
      2. Validator `overall: FAIL`, or any reviewer finding at `severity: BLOCKING` → **RETRY**, with one consolidated, numbered fix-list merging the review findings and the test failures (deduplicated).
      3. Any reviewer finding at `severity: MAJOR` → **RETRY** (folded into the fix-list) while the batch has retry budget. Budget exhausted → **ESCALATE** — unless you can state specifically why the finding is demonstrably non-blocking (correct by inspection, mechanism force-tested elsewhere); only then may it carry forward as a tagged note for the final review and PR. A MAJOR never carries silently, and BLOCKING never carries at all.
      4. Coder `DESIGN_CONFLICT`, or any reviewer finding that says a `DECISIONS.md` entry or ADR is wrong or must change → **ESCALATE**. Locked design decisions are the developer's seam, never an automated REPLAN.
      5. Evidence the decomposition itself is wrong — repeated retries hitting the same wall, a discovered dependency the plan missed, a batch that structurally can't leave tests green → **REPLAN**, with a one-paragraph statement of the broken assumption.
      6. Otherwise → **PROCEED**. `MINOR` findings: defer to a later subtask if one obviously fits, otherwise carry them forward as a note for the final review — don't block on them.

      Record `verdict` and a `rationale` that cites specific evidence (a finding, a `file:line`, a test name) — never a general impression. "Retry to be safe" burns budget on nits; "proceed to be nice" ships defects into the pre-PR gate. Budgets are checked mechanically in 5g regardless of what the rationale argues.
   e. Append the `batch_results` entry and persist the state file (disk only — never committed).
   f. Post the `BATCH_COMPLETE` record yourself (`internal/tracker-adapter/SKILL.md` → **Provenance records**) — subtasks, verdicts, any escalation decision resolved since the last one; it's the batch's provenance. Build it from the `batch_results` entry just written, not by re-reading the whole state file. On a write failure, record the error in the batch's `provenance` field, tell the developer at hand-off, and continue — but since this record is the provenance, more than one consecutive failure is worth an escalation rather than a silent gap in it.
   g. Act on the verdict:
      - **PROCEED** — mark the batch `DONE`, advance; after the last batch → FINAL REVIEW.
      - **RETRY** — if `retries_used[batch] + 1` would exceed `max_retries_per_batch` → ESCALATE instead, counter untouched (`validate-state.js` rejects a state where it's over). Otherwise increment and re-run from (a) with the fix-list (same base sha).
      - **REPLAN** — same check against `max_replans` (under `--turbo` it's `0`, so every REPLAN is an ESCALATE). Otherwise increment `replans_used`; `planner` re-decomposes the remaining subtasks; new batches get **fresh numbers, never reusing a dead batch's number** (retry budgets are keyed by batch number). Architect PLAN_REVIEW applies to the revised plan; `--gate replans` routes it to the developer instead. A rejection is an ESCALATE.
      - **ESCALATE** (including any coder `DESIGN_CONFLICT` — always an escalation, never a replan) — append to `escalations[]`, post the `ESCALATION` record (same skill section), ask the developer in chat (answer now, or save and come back later — see **Pausing and checkpoints**) and wait. If they save, or the session ends unanswered, checkpoint and set `phase: PAUSED`; `--resume` picks it up in any chat. The developer's decision is recorded and injected verbatim into the next coder/planner dispatch.
...
6. **FINAL REVIEW** (`phase: FINAL_REVIEW`) — the story-level pass the batches can't see.
   ...
   c. Decide the verdict exactly as in 5d, over the story-level results. **RETRY** → `coder` fixes (budget: `max_retries` for this phase, then escalate), then re-run (a)–(c). **ESCALATE** → as in Step 5g. **PROCEED** →
   ...
   e. **Collect the review notes:** every item that deserves a reviewer's deeper look — whether that reviewer is a human or a `/sdlc:review-run` pass — carried-forward MINOR findings, accepted-with-caveat decisions, assumptions the run made under its own judgment (from the designer's evidence list, verdict rationales, and reviewer notes). Store them in `final_review.notes`. Record `final_review.head_sha` (`git rev-parse HEAD`, `-C <worktree>` when set) — the exact diff this review passed on. Set `phase: READY_FOR_PR`, persist.
...
7. **PUSH + PR** (`phase: READY_FOR_PR` → `PR_OPENED`).
   ...
   - Post the review notes from 6e as a PR comment (line-anchored via the review-comments API where a note maps to a diff line; otherwise one summary comment), and post the `REVIEW_NOTES` record to mirror them onto the tracker story.
   - Record `pr: {number, url, base}`, set `phase: PR_OPENED`, persist.

8. **Discovered-work triage.** Throughout the run, the orchestrator accumulates `discovered_work` in the state file from every source — coder discoveries, reviewer findings judged out-of-scope, verdict carry-notes, designer open items deferred as future work. Now research each item before presenting it: when a real tracker is in normal write-mode, search it for existing tickets that already cover it (by symptom, package, and key terms — including the story's Depends-on neighbors); skip the search when `tracker: none` or the key is local — nothing to search. Present every item with options and a recommendation:
   - **New story** — nothing existing covers it; provide a draft summary + AC skeleton and the evidence.
   - **Comment on an existing ticket** — name the ticket found and why it's the right home.
   - **Drop** — with the reason it isn't worth tracking.
   The developer decides per item — **tickets are never created without their explicit choice.** On their decision: create the issue (linked to this story, backlog unless they say otherwise) or post the comment, and record the resolution in the hand-off summary.

9. **Hand off.** Print: the PR URL (or, under incognito, the branch and how to push it), subtasks completed, commits, budgets consumed, the design decisions made autonomously (with evidence pointers), the review notes, the discovered-work resolutions, and any provenance-write failures (gaps in the tracker/provenance record the developer should know about).
...
1. **AUTO_REVIEW** (`phase: AUTO_REVIEW`). **First, skip the redundant pass when nothing changed:** if the PR's head SHA (`gh pr view <PR#> --json headRefOid`) equals `final_review.head_sha`, FINAL_REVIEW already reviewed exactly this diff — record `auto_review: {verdict: "APPROVE", findings: [], reused_from: "FINAL_REVIEW"}` and continue to 2, after running step 2a's `/code-review` pass when `--review` is set (a different reviewer, not a repeat; any findings it raises make this "not clean" as below). Otherwise — the diff moved (a push, or a resume after the developer's fixes) — run `review-run`'s Mode A procedure against the PR, as `review-run <PR#> --review` when this run's own `--review` was set (otherwise without it) — a single-PR set, so its integrator cross-ticket pass is a no-op with nothing to compare against and is skipped, and with `--review`, step 2a's `/code-review` pass runs too, merged into the same finding set. Since the developer explicitly opted into "uninterrupted" via `--bypass`, treat the usual one-time batch-post confirmation as pre-authorized: post the review automatically rather than pausing for it. Record `auto_review: {verdict, findings, posted_at}`.
...
1a. **AUTO_FIX** (`phase: AUTO_FIX`). Run the `/sdlc:review-fix` procedure inline against the findings just recorded (see that command for the full contract — triage rules, loop mechanics, hard boundaries) — with this run's `review` state passed through the same way, so review-fix's own convergence check (its step 4e) also re-runs `/code-review` fresh when set, not just `reviewer`. This tail is review-fix's best case: `context: full` (the design note, context pack, and locked decisions all live in this run), so triage runs at full strictness rather than the mechanical-only fallback. Budget: **2 fix attempts for the whole tail**, tracked as `auto_review.fix_attempts` — attempts spent before a pause stay spent on resume. The phase is what legalizes the loop's pushes: `gate-git.js` allows `git push` at `AUTO_FIX` (and nowhere else in the tail). Its dispatches (`coder`, `validator`, `reviewer`) append to this run's `dispatches[]` — one run, one stats report.
   ...
   - **`ESCALATE`** (all-escalate triage, budget exhausted, or a coder `DESIGN_CONFLICT`) → report what was fixed (commits pushed so far stand) and what remains with the triage reasoning, checkpoint `phase: AUTO_REVIEW`, and stop — the same hand-off as before, now with the fix attempts on the record. (`--resume` re-runs the review fresh, since the developer's fixes will have changed the diff.)
...
**Collection is unconditional — it never depends on this flag.** Append a `dispatches[]` entry to `story-state.json` after **every** subagent dispatch across the whole run — Steps 2 through 7's `designer`/`architect`/`panelist`/`intake`/`planner`/`coder`/`reviewer`/`validator`, and, under `--bypass`, the `reviewer` (PR-mode) dispatch(es) in the AUTO_REVIEW/AUTO_FIX steps too — model tier, the **specific model version** actually running (e.g. "Opus 4.8" — read from the orchestrating session's own environment context at dispatch time, not from the tool result; see `internal/run-stats/schema.md`), wall-clock duration, tokens, and tool-call count, all already visible in that dispatch's own tool result (except the version, which the session already knows about itself). If a dispatch was given an explicit `model` override outside `opus`/`sonnet`/`haiku` (per the Model tiering table's own "pass a model override" note), record its role as `other` — one fixed color, never a new hue per override model — with `model_version` carrying the specific name. Phase timing needs nothing new — `phase_history[].started`/`.ended` already covers every phase including `DESIGN`, so "how much did the design phase take" is already answerable without new tracking. Collecting this costs nothing extra to dispatch — it's bookkeeping on data already in hand — so there's no reason to gate it behind a flag nobody remembers to pass.
...
```json
{
  ...
  "worktree_setup": {
    "source": "config|auto|asked|none — null when no worktree was created this run",
  ...
  "batch_results": [
    {"batch": 1, "attempt": 1,
     ...
     "provenance": {"status": "POSTED", "error": null}}
  ],
  "final_review": {
    "attempts": [{"review": {}, "validation": {}, "verdict": "PROCEED"}],
    "head_sha": "<HEAD the passing final review covered — lets the --bypass tail skip a redundant re-review>",
    "notes": [{"note": "...", "ref": "file:line or §", "kind": "MINOR|ASSUMPTION|CAVEAT"}]
  },
  "pr": {"number": null, "url": null, "base": null},
  "auto_review": {"verdict": null, "findings": [], "posted_at": null, "reused_from": null,
  ...
  "discovered_work": [
    {"description": "...", "evidence": "file:line or batch", "source": "coder|reviewer|verdict|designer|final_review",
  ...
  "gates": {
    ...
    "plan": {"status": "APPROVED|PENDING|REJECTED", "mode": "architect|human|turbo"},
    "replan_approvals": []
  },
  "escalations": [
...
| `reviewer` | `sonnet` | Catches design/correctness issues without paying Opus twice per batch; under `--bypass`, the same agent in PR mode runs the AUTO_REVIEW pass and AUTO_FIX's convergence re-reviews. |
| `validator` | `haiku` | Mechanical command-running and PASS/FAIL classification. |

Verdicts (5d/6c) and provenance posting are the orchestrating session's own work — no dispatch, no tier. To rebalance a single dispatch beyond what the resolved tier already does, pass a further `model` override on that Task call. (Installed marketplace plugins are read-only on the installee's machine.)

## Notes

- **Dispatch names are explicit.** Task tool `subagent_type`: `designer`, `architect`, `panelist`, `intake`, `planner`, `coder`, `reviewer`, `validator` — plugin-qualified `sdlc:<name>` if another plugin collides. Confirm once per machine via `/agents`.
...
- See `internal/tracker-adapter/SKILL.md` for the full tracker/write-mode resolution logic referenced throughout, `internal/model-effort/SKILL.md` for the full model-tier table and resolution, `internal/worktree-mode/SKILL.md` for the full worktree resolution/fallback/recovery logic, and `internal/local-docs/SKILL.md` for the full design-note/ADR locality logic — this file only describes how story-run *uses* each, not how they work.
