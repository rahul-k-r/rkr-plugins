...
5. **IMPLEMENT LOOP** (`phase: IMPLEMENT`). For the next batch with `status != DONE`:
   ...
   d. **Verdict — decided here, by the orchestrator, not a dispatch.** Every input is already in hand (plan, batch spec, coder output, reviewer findings, validator results, budgets — nothing to re-send), and you never saw the coder's reasoning either, only its structured output, so the blind-review property holds. Apply the procedure in order:
      1. Validator `overall: FAIL`, or any reviewer finding at `severity: BLOCKING` → **RETRY**, with one consolidated, numbered fix-list merging the review findings and the test failures (deduplicated).
      2. Any reviewer finding at `severity: MAJOR` → **RETRY** (folded into the fix-list) while the batch has retry budget. Budget exhausted → **ESCALATE** — unless you can state specifically why the finding is demonstrably non-blocking (correct by inspection, mechanism force-tested elsewhere); only then may it carry forward as a tagged note for the final review and PR. A MAJOR never carries silently, and BLOCKING never carries at all.
      3. Coder `DESIGN_CONFLICT`, or any reviewer finding that says a `DECISIONS.md` entry or ADR is wrong or must change → **ESCALATE**. Locked design decisions are the developer's seam, never an automated REPLAN.
