...
5. **IMPLEMENT LOOP** (`phase: IMPLEMENT`). For the next batch with `status != DONE`:
   ...
   f. Post the `BATCH_COMPLETE` record yourself (`skills/tracker-adapter/SKILL.md` → **Provenance records**) — subtasks, verdicts, any escalation decision resolved since the last one — to the tracker or appended to `provenance.md` per `write_mode`; it's the batch's provenance. Build it from the `batch_results` entry just written, not by re-reading the whole state file. On a write failure, record the error in the batch's `provenance` field, tell the developer at hand-off, and continue — but since this record is the provenance, more than one consecutive failure is worth an escalation rather than a silent gap in it.
