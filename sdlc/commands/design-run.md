...
0. **Preflight.** **Resolve the tracker and write-mode** per `skills/tracker-adapter/SKILL.md`: read `.sdlc/config.json` (or detect the registered MCP tool family and ask, offering to persist the choice). design-run doesn't take `--incognito` itself — when dispatched by `story-run` as its DESIGN phase, the tracker and write-mode were already resolved upstream and are passed down; use them as given, never re-derive. **When run standalone, resolve exactly as `story-run`'s own Step 0 does**: `tracker: none` resolves `writeMode` to `incognito` automatically, otherwise `normal`; and resolve `$1` per the Usage note above — a real tracker key fetched normally, or a no-ticket run per `story-run`'s **`--incognito` and no-ticket runs** section. Also resolve project config from the repo's `CLAUDE.md` — including an optional **design lens override**: a `CLAUDE.md` "design lenses" list replaces the default panel below. Run the **assignment check** (typo guard) exactly as `story-run` Step 1 defines it — **only when a real tracker key resolved; skipped entirely for a local key**, same as `story-run`'s own version (there's nothing to assign against a key with no tracker behind it).
   ...
   **Resolve `model_effort`** per `skills/model-effort/SKILL.md`: same inheritance rule as tracker/write-mode — dispatched inline by `story-run`'s DESIGN phase, it's already resolved by the parent run's own Step 0; inherit it as given, never re-resolve. Standalone: `--effort <tier>` if passed, else `.sdlc/config.json`'s `modelEffort`, else `high`. Every subagent dispatch below (`designer`, `architect`, `panelist`) passes an explicit `model` looked up from that skill's table for this tier — not restated at each dispatch below.

   **Resolve worktree mode** per `skills/worktree-mode/SKILL.md`: same inheritance rule — dispatched inline, `worktree` (and its branch) were already resolved and applied by the parent run's own Step 1; inherit as given. Standalone: `--worktree <path>` / `--no-worktree` / default (create one), exactly as `story-start.md`'s Step 6 resolves it. This session never calls `EnterWorktree` for any of the three modes. Resolved here but *applied* in Step 1, once the key's branch/slug are known.

   Initialize `docs/stories/<KEY>/story-state.json` (phase `DESIGN`, plus `tracker`, `write_mode`, `local_key`, `model_effort`) — untracked working state, same rules as story-run. (`worktree`, `branch`, and `base_branch` are recorded in Step 1, once resolved.)

1. **Branch/worktree setup, then design-note precedence.**
   - **Inline under `story-run`:** `worktree` (and `branch`/`base_branch`) were already resolved and applied by the parent run's own Step 1 — skip straight to the design-note check below, against that location.
   - **Standalone:** resolve the base branch exactly as `story-run`'s own `--base`-omitted resolution does (`branchModel: sprint` → auto-detect the highest-numbered `sprint/*` on origin, ask if ambiguous or absent; `branchModel: direct` → `main`), sync it (`git fetch origin && git switch <base> && git pull --ff-only`), then **apply worktree mode**: default → `git worktree add .claude/worktrees/<key-lower>-slug -b <branch> <base>` (branch named per `story-start.md`'s Step 6 convention, `feat|fix|chore/<key-lower>-slug`); `--worktree <path>` → verify it's a registered worktree (`git worktree list`) on the right branch, use as-is; `--no-worktree` → operate directly in the current checkout, no branch cut. Record `branch`, `base_branch`, and `worktree` (absolute path, or `null`) in the state file. **If `worktree` is non-null and `docs/design-notes/<KEY>.md` (and/or `docs/adr/`) already exist at this session's own root** (e.g. a prior `--no-worktree` design-run, or an interactive `/sdlc:plan-the-design` session) — copy them into the worktree now, before anything tries to commit them there, same as `story-run.md`'s Step 1.
   - **Either way:** if `docs/design-notes/<KEY>.md` (in `worktree` when set, else this session's own root) exists and passes the substantive checks (§1/§3/§5 filled), stop: the design is done — report that and point at `/sdlc:design-review` for critique. If it exists but is hollow, stop and tell the developer to finish or delete it — never silently redesign over a half-deliberated note.
...
5. **DRAFT.** Dispatch `designer` in drafting mode: the full note per `skills/design-review/template.md`, §3 carrying the deliberation (positions, dissents, decisions, invariants), ADR scaffolds for threshold decisions, §7 left empty. Written directly under `worktree` when set (else this session's own root) — the dispatch is told that working root explicitly, same as `coder`'s in story-run.
...
- Never writes application code, never transitions the tracker. Standalone, it cuts a branch only as a side effect of default worktree creation (Step 1) — skipped under `--no-worktree`, and never re-cut when inheriting an already-resolved branch/worktree from `story-run`. Outward-facing writes: the committed note/ADRs, the obligation comments (or provenance entries under `writeMode: incognito`), nothing else.
...
- See `skills/model-effort/SKILL.md` for the full model-tier table and resolution, and `skills/worktree-mode/SKILL.md` for the full worktree resolution/fallback/recovery logic — this file only describes how design-run *uses* each.
