...
1. **Identify the ticket and the mode.** Find the PR for the branch (`gh pr list --head <branch>` / `gh pr view`) and compare its author to the current user (`gh api user --jq .login`).
   - **PR author == you, or no PR yet → author mode.** Proceed read-only. Also check `docs/stories/<KEY>/story-state.json` for a `worktree` field (per `agent-plugin/skills/worktree-mode/SKILL.md`) — the file may not exist (this story never went through `/sdlc:story-start`'s worktree creation), in which case there's nothing to resolve. When it resolves to a path, every git inspection below (Steps 3 and 8) targets it via `git -C <worktree>` instead of the current checkout.
