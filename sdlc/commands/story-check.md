...
1. **Identify the ticket and the mode.** Find the PR for the branch (`gh pr list --head <branch>` / `gh pr view`) and compare its author to the current user (`gh api user --jq .login`).
   - **PR author == you, or no PR yet → author mode.** Proceed read-only.
   - **PR author != you → reviewer mode.** Fetch the PR (`gh pr checkout <number>` in a clean worktree, or `gh pr diff`) so the checks run against their work, then follow the reviewer-mode addendum after the standard steps.
