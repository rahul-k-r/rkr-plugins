...
4. **Check design note §7.** Read `docs/design-notes/$1.md` §7 (Implementation Findings) — from the session's own root under `local_docs: true`, `worktree` otherwise. If it is empty or still contains the template placeholder, warn: "§7 not filled — append implementation findings or confirm no material deviations."
...
6. **Post the completion record**, filled with the real evidence gathered above. **`write_mode: normal`:** post it via the tracker-adapter's `add_comment` op (`agent-plugin/skills/tracker-adapter/SKILL.md`). **`write_mode: incognito`:** append the same content as a new dated `COMPLETION_RECORD` entry to `docs/stories/<KEY>/provenance.md` instead (create the file with a one-line header if it doesn't exist yet) — never call a tracker write tool in this mode, regardless of what's registered.
