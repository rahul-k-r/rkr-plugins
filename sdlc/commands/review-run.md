...
- `--depth spot|full` — Mode B per-story verification depth (default `spot`): *spot* re-verifies each merged story's completion record and samples its AC evidence; *full* re-runs a complete `reviewer` pass on every merged story PR. Use `full` for high-stakes sprints or when spot checks surface smells.
- `--effort <tier>` — override `.sdlc/config.json`'s `effort` for this run only, per `internal/model-effort/SKILL.md` (`very-low`/`low`/`medium`/`high`/`extra-high`). Omitted → the config's `effort`, or `high` if that's unset too. Applies to either mode — every `reviewer`/`integrator` dispatch below passes an explicit `model` looked up from that skill's table for the resolved tier.
...
0. **Preflight.** `gh` authenticated; resolve the tracker per `internal/tracker-adapter/SKILL.md` (`.sdlc/config.json`, or detect the registered MCP family and ask — same resolution `story-run` uses); project config resolved from the target repo's `CLAUDE.md` (project key, verify commands, required CI check names, mandatory rules). **Resolve `effort`** per `internal/model-effort/SKILL.md`: `--effort <tier>` if passed, else `.sdlc/config.json`'s `effort`, else `high`. Every `reviewer`/`integrator` dispatch for the rest of this run passes an explicit `model` argument looked up from that skill's table for this tier — not restated at each dispatch below. Initialize `docs/stories/_reviews/<run-id>/review-state.json` (untracked — `docs/stories/.gitignore` covers it; run-id = sorted PR numbers or `sprint-<id>`), recording `effort` and `review` (whether `--review` was passed) in it.

1. **Resolve the review set — batched by stage across the whole set, not per PR** (a stage's calls all go in one message; each stage's inputs come only from the previous stage's outputs, so there's no ordering hazard): first, every `gh pr view --json number,title,author,baseRefName,headRefName,state,mergeable,statusCheckRollup` in one message; derive each story key from the PR title/branch; drop and report any PR that's closed, already merged (in Mode A), or key-less. Then, for the remaining PRs, every issue fetch (the adapter's `get_issue` op — when a tracker is configured, else skipped: under `tracker: none` the AC lives in the design note and/or the PR body, and `reviewer` is told so rather than being handed an empty AC as if it were real) plus design-note-path lookup, in one message. Then every Depends-on resolution (legacy IDs via the adapter's `search` op — the reviewer never touches the tracker itself; most batches have no legacy IDs and this stage is a no-op) in one message.

2. **Parallel per-PR reviews.** Dispatch one `reviewer` per PR in **PR mode**, **in a single message** (concurrent Task calls), each with: the PR number, story key + full AC (or design-note/PR-body AC under `tracker: none`), design note path, quoted mandatory repo rules, required CI check names, and the story's Depends-on keys from step 1. Reviewers work from `gh pr diff` + repo reads — **never `gh pr checkout`** (parallel checkouts would fight over the shared working tree). Each returns a structured verdict (`APPROVE`/`REQUEST_CHANGES`/`BLOCKED`), DoD checklist, findings, declared obligations, and Depends-on keys.

2a. **Code-review pass (only with `--review`).** For each PR in the batch, in turn (invoked inline via the Skill tool, not a Task dispatch — these run one at a time, unlike step 2's parallel `reviewer` calls): run `/code-review <PR#>` at low/medium effort. Add its findings into that PR's finding set from step 2, in the same shape (severity, file/line, suggested fix) `reviewer` already produces, tagged with `source: code-review` for the audit trail only — everything downstream (integration, consolidation, posting, triage) treats them identically to `reviewer`'s own findings.
...
2. **Per-story verification at `--depth`.** Parallel `reviewer` dispatches over the merged story PRs — *spot*: completion record exists and its evidence (CI run, merge SHA, AC mapping) checks out; *full*: complete re-review. Discrepancies (completion record claims vs. reality) are BLOCKING.
...
**Collection is unconditional — it never depends on this flag.** Append a `dispatches[]` entry to `review-state.json` after every subagent dispatch (`reviewer`, `integrator`, `validator`, in either mode; plus `coder` when `--fix` chains into the review-fix procedure) — model tier, the **specific model version** actually running (e.g. "Sonnet 5" — read from the orchestrating session's own environment context at dispatch time, not the tool result; see `internal/run-stats/schema.md`), wall-clock duration, tokens, and tool-call count. An explicit `model` override outside `opus`/`sonnet`/`haiku` (e.g. overriding `reviewer` for a high-stakes PR) records role `other` (one fixed color, never a new hue per override) with `model_version` carrying the specific name. `review-run` has no `phase_history` today — use coarse phase markers (`review`, `integration`, `gate`) for the report's phase timeline, or just the run's own start/end if that's all that's tracked.
...
**`--show-stats` controls only automatic publishing:** when passed, also render `internal/run-stats/template.html` (load the `artifact-design` and `dataviz` skills first, per the template's own instructions) filled in from the collected `dispatches[]`, publish it via the Artifact tool, and print the URL in the final report.
...
- Provenance: **GitHub reviews/comments on the PRs themselves** (the natural home for review records), the gate report embedded in the sprint PR body, and — **Mode A only, under the same `--post`/batch-confirmation gate as the GitHub reviews, never on its own** — a per-story tracker mirror (a `REVIEW_NOTES` record). Mode B never touches the tracker at all (consistent with "never transitions the tracker" — its record of what happened is the gate report in the sprint PR body). Nothing is committed to git by this command, ever.
...
| Agent | Model | Why |
|-------|-------|-----|
| `reviewer` (PR mode) | `sonnet` | Per-PR review runs N-wide in parallel; sonnet catches DoD/correctness/conformance issues at defensible cost. Override to `opus` at dispatch for high-stakes PRs or `--depth full` sprint audits. |
...
The Mode A tracker mirror (`REVIEW_NOTES`, gated by `--post`/batch confirmation) is the orchestrating session's own post — no dispatch, no tier.
...
- Dispatch names: `reviewer`, `integrator`, `validator`, and — via `--fix` only — `coder` (plugin-qualified `sdlc:<name>` on collision).
